...
/**
 * Simple network server implemented using jVerbs.
 * The server accepts a connection from a client, and sends a configurable number of messages,
 * after each message received from the client.
 * The messages the server sends contain a random integer, and another integer that was part of the
 * last message received.  In this way, whenever a message is received, the server can validate it
...
public class Server {
    ...
    public static final int N_MESSAGES = 1000000;

    /**
     * Runs the server.
     * The IP and port to bind the server can be provided as arguments.
     * By default, it binds to localhost on port 54321.
     *
     * @param args optional hostname and port to bind to.
     */
    ...
    public static final int BACKLOG_SIZE = 4;
    /** Size of a message: minimum two integers (2 * 4 bytes) */
    public static final int MESSAGE_SIZE = 8;

    /** Events channel for connection events. */
    private final EventChannel events;
    /** The connection accepting connection requests. */
    private final ConnectionId server;

    /** The connection with the client. */
    private ConnectionId client;
    ...
    /**
     * Creates a server object, and starts accepting connections on the given address.
     *
    ...
    public Server(InetSocketAddress addr) throws IOException {
        ...
        this.server = ConnectionId.create(this.events, PortSpace.RDMA_PS_TCP);
        this.server.bindAddress(addr);
        this.server.listen(Server.BACKLOG_SIZE);
        this.failed = false;
    }

    /**
     * Sets the failed state of this server.
     */
    ...
    /**
     * Waits for a client connection, initializes the verbs data structures, and accepts the
     * connection.
     *
     * @throws IOException if an I/O error occurs.
     */
    public void connect() throws IOException {
        ...
        this.client = event.getConnectionId();
        this.events.ackConnectionEvent(event);
        ...
        recvWR.setSgeList(new LinkedList<ScatterGatherElement>(Collections.singletonList(sge)));
        ...
        // accept connection
        this.client.accept(new ConnectionParameter());
        event = this.events.getConnectionEvent(-1);
        ...
        sendWR.setSgeList(new LinkedList<ScatterGatherElement>(Collections.singletonList(sge)));
     ...
     * Waits for N_MESSAGES messages from the client, and sends a reply after each message.
     * The messages the server sends contain a random integer, and another integer that was part of
     * the last message received.  In this way, whenever a message is received, the server can
     * validate it by checking if it contains the last random integer it sent.
     * Before sending the reply, the receive work request is reposted, unless there are no more
     * messages to receive.
     *
            ...
            while(this.pollCQ.execute().isSuccess() && this.pollCQ.getPolls() > 0) {
                ...
                if(wc.getOpcode() == WorkCompletion.Opcode.IBV_WC_SEND) {
                    ...
                    if(wc.getStatus() != WorkCompletion.Status.IBV_WC_SUCCESS) {
                        ...
                        // are there more messages to receive?
